#!/bin/bash
# Setup Olist database using psql directly
# This script bypasses Python and uses PostgreSQL CLI directly

# Configuration
POSTGRES_USER="${POSTGRES_USER:-postgres}"
POSTGRES_HOST="${POSTGRES_HOST:-localhost}"
POSTGRES_PORT="${POSTGRES_PORT:-5432}"
POSTGRES_DB="${POSTGRES_DB:-data_dictionary}"

echo "=========================================="
echo "Setting up Olist E-commerce Database"
echo "=========================================="
echo "Connection: $POSTGRES_USER@$POSTGRES_HOST:$POSTGRES_PORT/$POSTGRES_DB"
echo ""

# Create the SQL script inline
cat > /tmp/olist_setup.sql << 'EOF'
-- Create Olist Schema
...
psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -f /tmp/olist_setup.sql

if [ $? -eq 0 ]; then
    echo ""
    echo "=========================================="
    echo "✅ Olist Database Setup Complete!"
    echo "=========================================="
    echo ""
    echo "Schema: olist"
    echo "Tables: 9"
    echo "Sample Records: 52"
    echo "Indexes: 13"
    echo ""
else
    echo ""
